...
const Predict3 = () => {
  ...
  useEffect(() => {
    if (rawMaterials) {
      const prompt = JSON.stringify(rawMaterials);
      fetchData(prompt);
    }
  }, [rawMaterials]);

  const handleInputChange = (event) => {
    setRawMaterials(event.target.value.split(",").map((item) => item.trim()));
  };
  ...
  const handleSubmit = () => {
    if (rawMaterials.length > 0) {
      ...
      const prompt = `[
        ...
        //           "Unit": "PCS"
        //         },
        //     based on this data and the input of raw materials suggest what ItemName that can be made from the above dataset, just give 4 5 names no other text
        //       ]`;
      fetchData(prompt);
    }
  };
  ...
  return (
    <div className="formDiv">
      <input
        ...
        placeholder="Enter raw materials (separated by commas)"
      />
